...
- JSON Schema for `ansible_inventory.json` (the inventory artifact produced by IaC and consumed downstream) — including the
  `constants` block that carries the service / syslog / NetFlow / notification / vector-DB port values
...
| Single constants surface | Port values ride in the inventory's `constants` block (schema-validated), defined once upstream, never re-duplicated here |
| Breaking-change gate | CI diffs `schemas/` vs latest `versions/<vX.Y.Z>/` and requires a snapshot on shape change (ADR 0003); major bumps human-reviewed |
