...
- `schemas/ansible-inventory-v1.json` — JSON Schema (draft 2020-12) for the inventory artifact, including the `constants` block
  that carries the service / syslog / NetFlow / notification / vector-DB port values
...
| `dryvist/terraform-proxmox` | Defines `pipeline_constants` and writes them into the inventory's `constants` block per the schema; runs under `flow-lock` |
